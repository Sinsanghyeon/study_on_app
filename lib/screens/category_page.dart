...
class _CategoryPageState extends State<CategoryPage> {
  ...
  // [MODIFIED] Firestore 쿼리에서 제목 검색 부분을 제거합니다.
  Stream<QuerySnapshot> _buildStudyStream() {
    Query query = FirebaseFirestore.instance.collection('studies');

    // 제목 검색은 Firestore 쿼리에서 제외하고, 앱 내에서 처리합니다.
    // final searchText = _searchController.text.trim();
    // if (searchText.isNotEmpty) {
    //   query = query.where('title', isGreaterThanOrEqualTo: searchText).where('title', isLessThanOrEqualTo: '$searchText\uf8ff');
    // }
          ...
          Expanded(
            child: StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                // [MODIFIED] Firestore에서 가져온 데이터에 대해 앱 내에서 제목 검색을 수행합니다.
